...
class PageTree extends Component {
  ...
  onDrop = info => {
    if (info.dragNode) {
      ...
      const data = cloneDeep(this.props.interfaceDesign.initData);
      let dragObj = null; // 当前选中的节点元素
      loop(data, dragKey, (item, index, arr) => {
        arr.splice(index, 1);
        dragObj = item;
      });
      const dataRef = info.node.props.dataRef && info.node.props.dataRef;
      if (!info.dropToGap && isArray(dataRef.children)) {
